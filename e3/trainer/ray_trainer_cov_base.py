...
#     adv_seq = scores.clone()
#     for i, idx in enumerate(index):
#         adv_seq[i] -= torch.tensor(id2score[idx], device=scores.device).mean()

#     # compute kappa over correct-only (positive reward) samples per group
#     corrected_adv = adv_seq.clone()
#     for idx in set(index):
#         mask = torch.tensor([i for i, j in enumerate(index) if j == idx], device=scores.device)
#         adv_g = adv_seq[mask]
#         logp_g = seq_logp[mask]

#         # only use those with positive reward
#         correct_mask = (scores[mask] > 0)
#         if correct_mask.sum() < 2:
#             continue
#         a = adv_g[correct_mask]
#         lp = logp_g[correct_mask]

#         cov = ((a - a.mean()) * (lp - lp.mean())).mean()
#         var = (lp - lp.mean()).pow(2).mean()
#         kappa = cov / (var + epsilon)
#         kappa = torch.clamp(kappa, 0.0, kappa_clip)

#         corrected_adv[mask[correct_mask]] = a - kappa * lp

#     # tile back to token length
...
def compute_advantage(
    ...
    if adv_estimator == AdvantageEstimator.GAE:
        ...
        # advantages, returns, all_error_count = core_algos.compute_grpo_outcome_advantage(
        #     token_level_rewards=data.batch["token_level_rewards"],
        #     eos_mask=data.batch["response_mask"],
        ...
        advantages, returns = core_algos.compute_grpo_outcome_advantage(
            ...
            index=data.non_tensor_batch["uid"],
            norm_adv_by_std_in_grpo=True,  # or get from config
        )
        data.batch["advantages"] = advantages
        data.batch["returns"] = returns
        all_error_count=0
        ...
        data.batch["advantages"] = advantages
        data.batch["returns"] = returns
    elif adv_estimator == AdvantageEstimator.COVVAR:
        advantages, returns = core_algos.compute_covvar_token_vanilla_advantage(
            token_level_rewards=data.batch["token_level_rewards"],
            response_mask=data.batch["response_mask"],
        ...
        data.batch["advantages"] = advantages
        data.batch["returns"] = returns
    elif adv_estimator == AdvantageEstimator.DivGRPO:
        advantages, returns = core_algos.compute_dgrpo_advantage_outcome_A_mod(
            token_level_rewards=data.batch["token_level_rewards"],
            response_mask=data.batch["response_mask"],
            old_log_prob=data.batch["old_log_probs"],
            index=data.non_tensor_batch["uid"],
        ...
        data.batch["returns"] = returns
    # elif adv_estimator == AdvantageEstimator.DivGRPO:
    #     advantages, returns = core_algos.compute_divgrpo_outcome_advantage(
    #         token_level_rewards=data.batch["token_level_rewards"],
    #         response_mask=data.batch["response_mask"],
    #         old_log_prob=data.batch["old_log_probs"],
    #         index=data.non_tensor_batch["uid"],
    ...
    def fit(self):
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                print("Len rollout n prompt",self.rollout_n,len(batch_dict['rank']),[self.rollout_n]*len(batch_dict['rank']))
                batch = repeat(
                    batch.batch,
                    batch.non_tensor_batch,
                    batch.meta_info,
                    repeat_times=[self.rollout_n]*len(batch_dict['rank']),
                    interleave=True,
                )
